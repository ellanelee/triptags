...
import { IVenueAdminUpdate, IVenueDetailInput, IVenueDetailResponse } from "@triptags/shared"
...
export default function EditVenuePage({ params }: { params: { id: string } }) {
  ...
  const [venueData, setVenueData] = useState<IVenueAdminUpdate| null >(null)

  useEffect(() => {
    if (!isAuthenticated) router.replace("/venues")
    venue.run(() => venueApi.getVenueById(venueId))
    venueDetail.run(() => venueApi.getVenueDetail(venueId))
